...
async function fetchJimpImage(origin: string, relativePath: string, fallbackPath?: string): Promise<any> {
...
export const onGet: RequestHandler = async (requestEvent) => {
  ...
  try {
    ...
    const dest = new Jimp({ width: width * 60, height: height * 60, color: 0x00000000 });

    // 2. Load and crop background
    const background = await fetchJimpImage(origin, "/editor/portal/Images/portalBackground.jpg");
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const isFrame = y === 0 || y === height - 1 || x === 0 || x === width - 1;
