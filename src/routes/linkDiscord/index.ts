...
export const onGet: RequestHandler = async (requestEvent) => {
  ...
  if (!code) {
    throw requestEvent.redirect(302, `/profile/${userId}?error=discord_no_code`);
  }
  ...
  try {
    ...
    throw requestEvent.redirect(302, `/profile/${userId}?success=discord_linked`);
  } catch (err) {
    console.error("Discord OAuth failed", err);
    throw requestEvent.redirect(302, `/profile/${userId}?error=discord_link_failed`);
  }
};
